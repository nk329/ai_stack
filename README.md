...
점수제 매수 시스템 — **45점 이상**일 때 매수 신호 발생
...
| 레이어 | 주기 | 역할 |
|---|---|---|
| 모멘텀 스캔 | **10초** | 전체 200개 코인 현재가 조회 (API 1콜) → ±1.5% 이상 급등/급락 감지 |
| 핫 종목 분석 | **1분** | 감지된 상위 10개 코인 OHLCV 패턴 분석 → 즉시 매수 |
| 포지션 모니터 | **30초** | 현재가 기반 손절/익절/트레일링 즉시 반응 |
| 신호 체크 | **3분** | 보유 포지션 DCA·패턴매도 체크 |
| 심층 스캔 | **60분** | 전체 KRW 코인 재선발 |

### 4. 분할 매수 (DCA) + 3단계 분할 익절

```python
# DCA: BUY 신호 재발생 + 진입가 대비 -2~-7% 구간에서 추가 매수 (최대 1회)
if buy_signal and -0.07 <= drop_from_entry <= -0.02:
    → 추가 매수 (평단 낮추기)

# 3단계 분할 익절
+3%  → 보유수량 30% 매도  (1단계: 수익 확보)
+5%  → 남은수량 50% 매도  (2단계: 추가 확보)
나머지 → 트레일링 스탑     (3단계: 수익 극대화)
```

### 5. 동적 포지션 사이징 + 확신도 비례 투자

```python
# 동적 사이징: 가용 현금을 남은 슬롯에 균등 배분
invest = available_cash * 0.95 / remaining_slots

# 확신도 비례: 점수 높을수록 더 많이 투자
if score >= 70:  invest *= 1.3   # 강한 신호 → 30% 추가
elif score >= 55: invest *= 1.0  # 표준
else:             invest *= 0.8  # 약한 신호 → 20% 절감
```

### 6. 전체 KRW 코인 자동 스크리닝 (`data/screener.py`)
...
```
ai_stack/
├── scheduler.py          # 메인 오케스트레이터 (10초/1분/30초/3분/60분 스케줄)
...
- **모니터링 주기**: 모멘텀 10초 / 핫종목 1분 / 포지션 30초 / 신호 3분 / 스캔 60분
...
- 고정 타겟 3~7개 체계의 한계를 인식해 **10초 모멘텀 스캐너**로 전체 200개 코인 실시간 탐색으로 전환
- "가용 현금 75,000원이 놀고 있는 문제"를 진단 → 동적 포지션 사이징 + DCA + 3단계 분할익절로 자본 효율 극대화
